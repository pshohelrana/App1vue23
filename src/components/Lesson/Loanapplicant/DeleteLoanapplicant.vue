...
export default {
    methods: {
        async deleteItem() {
            ...
            const url = `${Settings.baseURL}/loanapplicants/${id}`; // Adjust the URL as per your API

            try {
                const response = await Http.delete(url);
                console.log('Loanapplicant deleted:', response);

                // Optionally, you can navigate the user to another page after deletion
                // Example: this.$router.push('/items');
            } catch (error) {
                console.error('Error deleting loanapplicant:', error);
            }
        }
    }
}
